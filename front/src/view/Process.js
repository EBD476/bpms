...
const items = [
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: {
        compare: (a, b) => a.processDefinitionKey - b.processDefinitionKey,
        multiple: 3,
      },
    },
    {
    ...
    // },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <a href={`/processDetails/${record.id}`}>Details</a>
          <Dropdown
           row={record.id} 
           trigger={['click']}
           menu={{
...
const App: React.FC = () => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {                
        try {
          const response = await fetch('http://localhost:9090/api/process/running-processes', {
            method: 'GET',            
          });                          
